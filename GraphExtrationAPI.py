...
@app.route("/debitbalance", methods = ['GET','POST'])
def debitbalance():
    target_path ='D:\\uploads'
    folder_path = r'D:\\uploads'
    file_type = '\*csv'
   ...
   # import_file =import_file.iloc[: , :-1]
    ...
    plt.xlabel('Debit')
    plt.ylabel('Balance')
    plt.title('Debit Balance Analysis')
    plt.legend()
    plt.show()
    fig.savefig('D:\DebitAnalysis\saved_figure.png')
    file = "saved_figure.png"
    return send_file("D:\DebitAnalysis\saved_figure.png",mimetype=("image/png"))
